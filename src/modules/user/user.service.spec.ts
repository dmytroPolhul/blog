...
describe('UserService', () => {
  ...
  it('should create new User', async () => {
    ...
    expect(mockRepository.findOne).toHaveBeenCalledWith({ where: {email: userPayload.email }});
    expect(mockRepository.create).toHaveBeenCalledWith({...userPayload, password: 'hashedPassword'});
  ...
  it('should return bad request error', async () => {
    ...
    await expect(service.createUser(userPayload))
        .rejects
  ...
  it('should get user by id', async () => {
    ...
    const result = await service.getUser("3247aca5-4e16-4fa1-a1da-35203bb3313a");
  ...
  it('should soft delete user by id', async () => {
    ...
    mockRepository.delete.mockResolvedValueOnce({...updatedUser, deletedAt: "2023-08-12T16:04:23Z"});

    const result = await service.deleteUser("3247aca5-4e16-4fa1-a1da-35203bb3313a");
    expect(result).toEqual({...updatedUser, deletedAt: "2023-08-12T16:04:23Z"});
  });
  ...
  it('should update user info with password and email address', async () => {
    ...
    mockRepository.findOneOrFail.mockResolvedValueOnce({...userResponse, password: 'hashedPassword'});
    mockRepository.findOne.mockResolvedValueOnce(null);
  ...
  it('should return bad request when comparing old passwords is falls', async () => {
    ...
    await expect(service.updateUser(request))
        .rejects
        .toThrow(BadRequestException);
